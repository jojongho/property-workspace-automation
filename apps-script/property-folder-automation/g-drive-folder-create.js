 ...
 *
 * ✅ 통합 폴더 구조 (유형 최상위 분리):
 * - 모든 매물 유형: [유형 루트]/시군구/동읍면/통반리/[매물별 규칙]
 * 
 * 매물별 하위 구조 (기존 규칙 유지):
...
function getTypeRootFolder_(typeKey) {
  var rootFolder = getRootFolder_();
  var folderName = TYPE_ROOT_FOLDER_NAMES[typeKey];
  if (!folderName) {
    throw new Error('유형 루트 폴더명이 정의되지 않았습니다: ' + typeKey);
  }

  return getOrCreateFolder(rootFolder, folderName);
}

function getTypeRegionalParentFolder_(typeKey, 시군구, 동읍면, 통반리) {
  var typeRootFolder = getTypeRootFolder_(typeKey);
  var normalized시군구 = normalize시군구(시군구);
  var 시군구Folder = getOrCreateFolder(typeRootFolder, normalized시군구);
  var 동읍면Folder = getOrCreateFolder(시군구Folder, 동읍면);

  if (통반리 && String(통반리).trim() !== '') {
    return getOrCreateFolder(동읍면Folder, 통반리);
  }
...
function getApartmentParentFolder_(시군구, 동읍면, 통반리) {
  return getTypeRegionalParentFolder_('apartment', 시군구, 동읍면, 통반리);
}
...
function createTownFolderStructure(시군구, 동읍면, 통반리, 지번, 주택단지, 주택유형, 동, 호, 타입) {
  let 부모Folder = getTypeRegionalParentFolder_('town', 시군구, 동읍면, 통반리);
...
function createBuildingFolderStructure(시군구, 동읍면, 통반리, 지번, 건물명, 매물유형, 호, 상호명, 거래유형, 방구조) {
  ...
  let 부모Folder = getTypeRegionalParentFolder_('building', 시군구, 동읍면, 통반리);
...
function createLandFolderStructure(시군구, 동읍면, 통반리, 지번, 토지분류) {
  let 부모Folder = getTypeRegionalParentFolder_('land', 시군구, 동읍면, 통반리);
...
function createFactoryFolderStructure(시군구, 동읍면, 통반리, 지번, 명칭) {
  let 부모Folder = getTypeRegionalParentFolder_('factory', 시군구, 동읍면, 통반리);
